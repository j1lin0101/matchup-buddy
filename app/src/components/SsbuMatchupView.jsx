...
// (see CharacterSelect.jsx / useMatchupData.js / scripts/fetch-ssbu-moves.js).
...
function CharColumnHeader({ name, accent }) {
  ...
  const wikiUrl = `https://dragdown.wiki/wiki/SSBU/${name.replace(/ /g, '_')}`
  return (
    <a
      href={wikiUrl}
      target="_blank"
...
function ComboBreakersList({ charData }) {
  const breakers = useMemo(() => getComboBreakers(charData), [charData])
  if (!breakers.length) return <EmptyNote>No data.</EmptyNote>
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {breakers.map((b, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontWeight: b.isDefensive ? 700 : 600, flexShrink: 0 }}>{b.label}</span>
          <span style={{ flex: 1 }} />
          <FrameBadge frames={b.startup} color={b.isDefensive ? 'var(--accent2)' : 'var(--accent)'} />
        </div>
      ))}
    </div>
  )
        ...
        {activeTab === 'overview' && (
          <div className="top-panels-grid">
            ...
            <div className="char-panel-oos-my">
              {myData
                ? <Section title="Fastest OOS Options" accent="var(--accent)" tooltip="Fastest options available out of shield, sorted by total frames from shielding to the move hitting.">
            ...
            <div className="char-panel-oos-opp">
              {oppData
                ? <Section title="Fastest OOS Options" accent="var(--accent2)" tooltip="Fastest options available out of shield, sorted by total frames from shielding to the move hitting.">
                    <OOSList charData={oppData} />
                  </Section>
                : null}
            </div>

            <div className="char-panel-combo-my">
              {myData
                ? <Section title="Combo Breakers" accent="var(--accent)" tooltip="This character's fastest escape options out of a combo: airdodge, jump, and any move faster than both.">
                    <ComboBreakersList charData={myData} />
                  </Section>
                : null}
            </div>
            <div className="char-panel-combo-opp">
              {oppData
                ? <Section title="Combo Breakers" accent="var(--accent2)" tooltip="This character's fastest escape options out of a combo: airdodge, jump, and any move faster than both.">
                    <ComboBreakersList charData={oppData} />
                  </Section>
                : null}
            </div>
          </div>
